...
func (th *TradeshiftHandler) ImportFeedToTradeshift(
	...
	//upload file
	r, err := api.UploadFile(reportPath)
	if err != nil {
		return err
	}
	...
	log.Printf("Import was completed with state: %v", state)

	//get import report
	report, err := api.GetImportResult(actionID)
	if err != nil {
		return err
	}

	th.handler.Write(th.filemanager.BuildTradeshiftImportResultsPath(feedPath), [][]string{{report}})
	return nil
}
...
func (th *TradeshiftHandler) waitForImportComplete(actionID string) (string, error) {
	...
	for currentState == inProgressImportState {
